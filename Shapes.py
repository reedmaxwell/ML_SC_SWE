...
def conv1d_shape(input_shape=[1, 5], cout=16, padding=1, kernel_size=3, dilation=1, stride=1):
    #Function to calculate the output shape of a conv2D layer given the shape of the
    #input array and the model settings.
    # Conv1D input shape: (Cin, Lin)
    # Conv1D output shape: (Cout, Lout)
    # where:
    # N  = Batch Size
    # Cin = # of channels input and Cout=#channels output
    # Lout = [Lin + 2xpadding - (dilation x (kernel_size - 1)) - 1] / stride +1]
    Lin = input_shape[1]
    # 
    Lout = (Lin + 2 * padding - dilation *(kernel_size - 1) - 1) / stride + 1
    output_shape = [cout, Lout]

    return(output_shape)

def pool1d_shape(input_shape=[1, 5], padding=0, kernel_size=2, dilation=1, stride=1):
    ...
    # N  = Batch Size
    # Cin = # of channels input and Cout=#channels output
    # Lout = [Lin + 2xpadding[0] - dilation[0]x(kernel_size[0] - 1) - 1]/stride[0] + 1
    Lin = input_shape[1]
   
    Lout = (Lin + 2 * padding - dilation *(kernel_size - 1) - 1) / stride + 1
    
    output_shape = [input_shape[0], Lout]
