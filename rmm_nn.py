...
class RMM_NN(nn.Module):
    def __init__(self, grid_size=[8760, 1],  channels=3, verbose=False,):
        ...
        # Shape calculations
        # ---------------------------------------------------------------------
        c1d_s1 = shp.conv1d_shape(input_shape=in_shape, cout=Cout, padding=cnv_padding,
                              kernel_size=cnv_kernel_size, dilation=cnv_dilation, stride=cnv_stride)
        c1d_s2 = shp.conv1d_shape(input_shape=c1d_s1, cout=Cout, padding=cnv_padding,
                              kernel_size=cnv_kernel_size, dilation=cnv_dilation, stride=cnv_stride)

        pool1d_s1 = shp.pool1d_shape(input_shape=c1d_s2, padding=pool_padding, kernel_size=pool_kernel_size,
                                 dilation=pool_dilation, stride=pool_stride)
        c1d_s3 = shp.conv1d_shape(input_shape=pool1d_s1, cout=Cout, padding=cnv_padding,
                              kernel_size=cnv_kernel_size, dilation=cnv_dilation, stride=cnv_stride)
        pool1d_s2 = shp.pool1d_shape(input_shape=c1d_s3, padding=pool_padding, kernel_size=pool_kernel_size,
                                 dilation=pool_dilation, stride=pool_stride)
        ...
        # Linear Steps
        # ---------------------------------------------------------------------
        L_Fin1 = int(np.prod(np.floor(pool1d_s1)))
